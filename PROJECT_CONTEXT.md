...
4. **Hospital incoming-ambulance notifications require a manual refresh** — should update automatically. Celery/WebSockets/Redis were deliberately removed from this project early on, so the realistic fix is frontend polling (periodic re-fetch), not true real-time push — confirm this trade-off is acceptable before implementing.
