...
**This is a deliberate, temporary state.** The team has not yet decided whether to do a hard rename (`hospital` → `hospital_admin`, `ambulance_service` → `ambulance_admin`) across every file that references the old names, or to migrate references gradually. **Do not assume `hospital` and `hospital_admin` are interchangeable in existing code** — as of this writing, `IsHospital`, `IsAmbulanceService`, and filters like `role='hospital'` in `HospitalListView` still only check the *old* role names, so accounts created with the *new* role names will currently fail those checks. This needs to be resolved before hospital_admin/ambulance_admin accounts can actually use existing verification/emergencies functionality.
...
- `accounts` — custom `User` model with a `role` field. Currently holds both old (`hospital`, `ambulance_service`) and new (`hospital_admin`, `ambulance_admin`, `emt`, `mera_admin`) role values simultaneously — see the "Role naming" section above before changing anything here
- `medical_profiles` — patient medical data, includes a `data_sharing_consent` field (for hospital/ambulance access) that is **separate** from `ai_chatbot_consent` (for AI chatbot use) — these must never be conflated
...
- Don't assume `hospital`/`hospital_admin` or `ambulance_service`/`ambulance_admin` are interchangeable in existing permission checks — they currently are NOT (see "Role naming" section above). Flag this explicitly if it affects the task at hand rather than silently assuming one or the other.
